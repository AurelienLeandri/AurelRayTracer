...
void ApplicationOld::processKeyboard(CameraMovement direction, float deltaTime)
{
...
void ApplicationOld::processMouseMovement(float xoffset, float yoffset)
{
...
void ApplicationOld::updateCameraVectors()
{
...
bool ApplicationOld::_processInput()
{
...
bool ApplicationOld::refreshWindow(const float* image_buffer)
{
...
int ApplicationOld::_openglInit() {
    glfwInit();
...
int ApplicationOld::_vulkanInit() {
    createInstance();
    return 0;
}

void ApplicationOld::run()
{
    if (!initWindow()) {
        _vulkanInit();
        setupDebugMessenger();
...
namespace {
    void framebufferResizeCallback(GLFWwindow* window, int width, int height) {
        ApplicationOld* app = (ApplicationOld*)(glfwGetWindowUserPointer(window));
...
void ApplicationOld::createInstance()
{
...
void ApplicationOld::setupDebugMessenger() {
...
void ApplicationOld::pickPhysicalDevice()
{
...
void ApplicationOld::createLogicalDevice() {
...
void ApplicationOld::createSurface() {
    if (glfwCreateWindowSurface(instance, _window, nullptr, &surface)) {
        throw std::runtime_error("Failed to create window surface!");
    }
}

ApplicationOld::QueueFamilyIndices ApplicationOld::findRequiredQueueFamilies(VkPhysicalDevice device) {
...
int ApplicationOld::ratePhysicalDevice(VkPhysicalDevice device) {
    ...
    if (!deviceFeatures.geometryShader) {
        // ApplicationOld can't function without geometry shaders
        return 0;
    }
...
VkSurfaceFormatKHR ApplicationOld::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>&availableFormats) {
...
VkPresentModeKHR ApplicationOld::chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) {
...
VkExtent2D ApplicationOld::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
...
bool ApplicationOld::checkInstanceRequiredExtensionsSupport(const std::vector<const char*> &requiredExtensions) {
    uint32_t extensionCount = 0;
    vkEnumerateInstanceExtensionProperties(nullptr, &extensionCount, nullptr);
...
bool ApplicationOld::checkDeviceRequiredExtensionSupport(VkPhysicalDevice device) {
    uint32_t extensionCount = 0;
...
bool ApplicationOld::checkValidationLayerSupport(const std::vector<const char*>& requiredLayers) {
    uint32_t layerCount;
...
void ApplicationOld::createSwapChain() {
...
ApplicationOld::SwapChainSupportDetails ApplicationOld::querySwapChainSupport(VkPhysicalDevice device) {
...
void ApplicationOld::createImageViews() {
...
void ApplicationOld::createRenderPass() {
...
void ApplicationOld::createGraphicsPipeline() {
...
VkShaderModule ApplicationOld::createShaderModule(const std::vector<char>& code) {
...
void ApplicationOld::createFramebuffers() {
...
void ApplicationOld::createCommandPool() {
    QueueFamilyIndices queueFamilyIndices = findRequiredQueueFamilies(physicalDevice);
...
void ApplicationOld::createCommandBuffers() {
...
void ApplicationOld::createSyncObjects() {
...
void ApplicationOld::recreateSwapChain() {
...
void ApplicationOld::cleanupSwapChain() {
...
uint32_t ApplicationOld::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
...
void ApplicationOld::createVertexBuffer() {
...
void ApplicationOld::createIndexBuffer() {
...
void ApplicationOld::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
...
void ApplicationOld::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage,
...
void ApplicationOld::createDescriptorSetLayout() {
...
void ApplicationOld::createUniformBuffers() {
...
void ApplicationOld::updateUniformBuffer(uint32_t currentImage) {
...
void ApplicationOld::createDescriptorPool() {
...
void ApplicationOld::createDescriptorSets() {
...
void ApplicationOld::createTextureImage() {
...
void ApplicationOld::generateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels) {
...
void ApplicationOld::createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
...
VkCommandBuffer ApplicationOld::beginSingleTimeCommands() {
    VkCommandBufferAllocateInfo allocInfo{};
    allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
    allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
    allocInfo.commandPool = commandPool;
...
void ApplicationOld::endSingleTimeCommands(VkCommandBuffer commandBuffer) {
...
void ApplicationOld::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
void ApplicationOld::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height) {
...
void ApplicationOld::createTextureImageView() {
    textureImageView = createImageView(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, mipLevels);
}

VkImageView ApplicationOld::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels) {
...
void ApplicationOld::createTextureSampler() {
    ...
    // TODO: Store this once we picked the physical device, to avoid calling it all the time
...
VkFormat ApplicationOld::findDepthFormat() {
...
void ApplicationOld::createDepthResources() {
...
VkSampleCountFlagBits ApplicationOld::getMaxUsableSampleCount() {
...
void ApplicationOld::createColorResources() {
