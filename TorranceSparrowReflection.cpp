...
glm::vec3 TorranceSparrowReflection::f(const glm::vec3& w_i, const glm::vec3& w_o, const HitRecord& hit_record) const
{
	...
	float fresnel_w_o = (_k != 0 ? FresnelSpecular::fresnelConductor(cos_w_i_w_h, _etaRay, _etaInterface, _k) : FresnelSpecular::fresnelDielectric(cos_w_i_w_h, _etaRay, _etaInterface));
...
glm::vec3 TorranceSparrowReflection::sample_f(glm::vec3& w_i, const glm::vec3& w_o, const HitRecord& hit_record, float& pdf) const
{
	...
	if (glm::dot(w_o, w_h) < 0)  // wço has to point toward the surface of the microfacet
		return glm::vec3(0, 0, 0);
